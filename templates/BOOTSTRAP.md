...
메시지가 `워크플로워 ROLE_ACTIVATE`로 시작하면 원격 활성화다. 아래 5개 역할 파싱을 건너뛰고, 메시지의 ROLE에 해당하는 스킬만 읽은 뒤 `ROLE_READY`를 RETURN_TO로 반환한다.
...
오케스트레이터가 통신 어댑터를 선택한 뒤 전체 endpoint를 확인한다.

각 역할 담당자는 자신의 endpoint와 반환 대상만 확인한다.
...
DEFERRED
- architect until task request
- plan reviewer until plan ready
- implementer until plan pass and TDD assessment
- implementation reviewer until implementation complete
```
