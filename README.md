...
기획·설계에서 사용자의 의도를 정확히 받아내기 위한 스킬이다. **요청할 때만 로드된다.**

작업 지시에 인터뷰를 요청하면 오케스트레이터가 그 요청을 아키텍트에 실어 보내고, 아키텍트가 스킬을 로드해 사용자와 직접 문답한다. 오케스트레이터는 질문을 중계하지 않는다. 무엇을 물을지 고르는 것 자체가 기술적 판단이라 오케스트레이터의 금지 사항에 걸린다.

```text
워크플로 상태를 세션 간에 복구할 수 있게 해줘.
인터뷰하면서 설계할 것.
```

이번 요청에만 적용된다. 다음 작업에서 요청하지 않으면 아키텍트는 스킬 없이 평소대로 설계한다.

질문은 한 번에 하나다. 선택지는 최대 3개이고, 없어도 된다.
...
`PRINCIPLES.md`는 읽기 쉬움 우선, YAGNI, 여백과 구조 원칙을 담는다. 오케스트레이터를 뺀 네 역할이 읽는다. 아키텍트는 설계에, 계획 리뷰어는 계획 검증에, 임플리먼터는 코드 작성에, 구현 리뷰어는 코드 검증에 적용한다.

오케스트레이터는 기술적 판단을 하지 않으므로 적용 대상이 아니다.

저장소마다 조정할 수 있고 업데이트가 덮어쓰지 않는다.
...
역할의 작업 완료와 차단 보고는 자기 pane에만 남기지 않는다. 현재 작업 메시지의 `RETURN_TO` endpoint로 상태 메시지를 보내 상대 pane의 새 입력으로 제출한다. 상대 에이전트는 이 입력으로 깨어나 다음 단계로 진행하며, 화면 polling은 누락 복구에만 사용한다.
...
1. 현재 Git 저장소 루트 확인
2. `CONFIG.md`, `PRINCIPLES.md`, `STATE.md`, `ARCHI.md` 확인
3. 사용자 메시지에서 5개 역할 배정 파싱
4. 탭 라벨로 배정 이름을 endpoint에 매핑
5. 현재 단계에 필요한 역할 스킬만 로드
6. 같은 에이전트가 복수 역할이면 단계별로 전환
7. 각 역할의 `ROLE_READY`는 최초 활성화 시 확인
8. 오케스트레이터가 `WORKFLOW_READY` 표 보고
9. 다음 사용자 작업 지시 전까지 설계·리뷰·구현 금지

## 역할 식별

배정 이름을 endpoint로 해석하는 기준은 **탭 라벨 하나뿐이다.**

pane에서 어떤 CLI가 어떤 모델로 도는지는 라우팅에 쓰지 않는다. `herdr pane list`의 `agent` 필드는 `opencode`까지만 알려주므로 같은 CLI를 쓰는 두 pane을 구분하지 못하고, 상태줄 모델명은 문구가 바뀌면 깨진다. 라벨만 유지되면 모델을 바꾸거나 CLI를 갈아끼워도 워크플로는 그대로 동작한다.

탭 라벨을 배정 이름에 맞춘다.

```bash
herdr tab rename w1:t2 codex-luna
```

한글과 영문, 부분 이름의 대응은 에이전트가 판단한다. `프로`와 `opencode-pro`, `코덱스솔`과 `sol`은 같은 대상이다. 하나로 확정되지 않으면 추측하지 않고 `WORKFLOW_BLOCKED`로 사용자에게 묻는다.
...
- `.ai/workflow/PRINCIPLES.md` — 없을 때만 생성
- `.ai/workflow/STATE.md` — 없을 때만 생성
- `.ai/memory/ARCHI.md` — 없을 때만 생성
- `AGENTS.md`, `CLAUDE.md`의 최소 부트스트랩 블록

기존 `CONFIG.md`, `PRINCIPLES.md`, `STATE.md`, `ARCHI.md`는 덮어쓰지 않는다.
